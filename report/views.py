...
from hotfileloader import process_billing_details_from_hotfile,process_billing_details_from_hotfile_uk
# 3rd-party imports
...
class ReportUpload(PermissionRequiredMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context[' '] = is_arc(self.request.session.get('country'))
    ...
    def form_valid(self, form):
        ...
        def thread_process(self, country, form, file, filepath, period,filenw=None, from_zip=False):
            print(period,"piribjuytrfthftfdtd/////////////////////////////////////////////////////////////////////////////////////////")
            ...
            if re.search("^[0-9]+$", extention) and country.name == "United Kingdom":

                error= process_billing_details_from_hotfile_uk(text_file, self.request,period)
                
            
            elif re.search("^[0-9]+$", extention) and country.name !='United States':
        ...
        if extention.lower() == 'zip':
            ...
            for name in filenw.namelist():
                ...
                if name.endswith('/'):
                    ...
                    process = threading.Thread(target=thread_process,
                                               args=(self, country, form, file_data, filepath, None, filenw, from_zip))
                    process.start()
                    process.join()
            ...
            if re.search("^[0-9]+$", extention) and country.name != 'United States':   
                ...
                if country.name == 'United Kingdom':
                    ...
                    period = file_name.split(".")[3][4:]
                    
                    print("kkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk")
                    process = threading.Thread(target=thread_process, args=(self, country, form, file, filepath, period))
                    process.start()
            ...
            process = threading.Thread(target=thread_process, args=(self, country, form, file, filepath, None))
            print("?????????????????\\\\\\\\\\\\\\\?????????????????????")
            process.start()
        print("..........END............")       
                ...
                for t in transactions:
                    ...
                    transactions_rows.append(
                        {"fare": total_fare_amount_var,"Transaction_amount":total_transaction_amount_var, "tax": t.get('total_tax'),
                         ...
                         "comm": (abs(total_commission_var) * -1), "pen": t.get("pen_value"),"tax_on_commission": tax_on_commission_var,
                         "tax_yq_yr": total_tax_yq_yr, "tax_cp_mf": total_tax_cp_mf, "weekly_sales_total": weekly_sales,
                         "total_ca": cash_disbursement, "total_cc": t.get("report__cc"), "Easy_pay": Easy_pay_var,
                         "total_ap_acm": total_ap_acm_var, "remittance": remittance,
                         "week": t.get('report__report_period__week'), "pending_deduction": pending_deduction,
                         'net_sales_weekly': net_sales_weekly, 'ped_total': total_fare_amount_var})
                ...
                for t in transactions:
                    ...
                    try:
                        pending_deduction = Deduction.objects.filter(
                            ...
                            pending=True).aggregate(pending_deduction=Coalesce(Sum('amount'), V(0))).get(
                            'pending_deduction')
                        print(pending_deduction,"..............pending_deduction...............")
                    except Exception as e:
                        pending_deduction = 0.00
    ...
    def get_context_data(self, **kwargs):
        ...
        if airline and month_year:
            ...
            if is_arc_var:
                ...
                total_Transaction = Transaction.objects.filter(report__airline = airline_data,report__report_period__month = month, report__country = country )
                ...
                for t in transactions:
                    ...
                    Total_comm = Total_comm + (abs(total_commission_var) * -1)
            ...
            print(pending_deduction_total,"............pending_deduction_total................")
    ...
    def get(self, request, *args, **kwargs):
        ...
        if airline and month_year:
                ...
                for t in transactions:
                    ...
                    transactions_rows.append(
                        {"fare": total_fare_amount_var, "Transaction_amount":total_transaction_amount_var, 
                         ...
                         "Total_tax":Total_tax, "Total_fare": Total_fare, "Total_ca": Total_ca, "Total_cc": Total_cc })
            ...
            for ele in transactions_rows:
                ...
                if ele.get("pending_deduction") is None:
                    ws.write(row, 11, (ele.get("pending_deduction")),center_normal)
                else:
                    ws.write(row, 11,round(ele.get("pending_deduction"),2),center_normal)
                ...
                if ele.get("pending_deduction_total") is None:
                    ws.write(row_num, 11, (ele.get("pending_deduction_total")), center_normal_border_yellow_center)
                else:
                    ws.write(row_num, 11, round(ele.get("pending_deduction_total"),2), center_normal_border_yellow_center)
            ...
            ws.write_merge(34, 35, 0, 3, "ACSA  Fee (" + str(iata_coordination_fee) + " %)",left_normal_bold)
