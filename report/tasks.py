...
def create_charges_and_taxes(transaction_details_values, transaction, countryname=""):
    ...
    print(transaction_details_values,"/*/*/***/*//*/")
...
def create_transaction(agency, rf, transaction_details_values, countryname=None):
    ...
    if countryname and countryname.lower() in ["netherlands",
        ...
        print(transaction_details_values.get('std_comm_amount'),"floated----")
    ...
    try:
        ...
        issue_date= transaction_details_values['issue_date']
...
def process_billing_details(text_file=None, request=None, report_file=None):
    ...
    if report_file:
        ...
        if request:
            ...
            country = None
    if m:
        header_values = m.groupdict()##code of air, air name
        try:
    ...
    if not report_file:
        number, line = next(reader, None)## reader is all lines and next will take the next line which took before

        m = re.match(period_header, line)##got out the period like 08-APR-2022 to 15-APR-2022
        period_values = None
        while not m:
            number, line = next(reader, None)
...
class ProcessData(object):
    ...
    def create_transaction(self, transaction_details_values):
        transaction = Transaction.objects.create(
            ...
            issue_date=str(transaction_details_values.get('issue_date', self.date)),
